...
with tab1:
    ...
    # https://www.youtube.com/watch?v=vTScJ6Xo_hQ # line chart
    # https://plotly.com/python/axes/ # axes
    # use autoscale to see the whole chart
    df1 = pd.read_excel('price_analysis_product_data.xlsx')
    fig1 = px.line(df1,x='日期',y='數值',color='種類',title='CPI折線圖(以商品性質分類) - 113年1月至114年1月',labels={'數值': 'CPI'})
    ...
    with expander_source1:
        st.write('https://dmz26.moea.gov.tw/GA/common/Common.aspx?code=K&no=1')
...
with tab2:
    ...
    with popover:
        st.write('在圖表右上方可以將圖表放大')
    df2 = pd.read_excel('price_analysis_product_rate_data.xlsx')
    fig2 = px.line(df2,x='日期',y='數值',color='種類',title='CPI年增率折線圖(以商品性質分類) - 113年1月至114年1月',labels={'數值': 'CPI年增率'})
